    ...
    public func addVertice(_ vertice: SCNVector3) {
        ...
        if verticesPool.count < 3 {
            ...
            for vertice in verticesPool {
                smoothed = smoothed + vertice
            }
            smoothed = SCNVector3(smoothed.x / Float(verticesPool.count), smoothed.y / Float(verticesPool.count), smoothed.z / Float(verticesPool.count))
